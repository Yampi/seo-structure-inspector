<?php
/**
 * BaloaStructureAuditorSEO\Services\AI\OpenAIProvider
 *
 * OpenAI GPT-4o provider for AI recommendations.
 */
...
class OpenAIProvider implements AIProviderInterface {
    ...
    public function get_recommendations( string $url, array $context = [] ): array {
        ...
        if ( is_wp_error( $response ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'OpenAI API request failed', [ 'error' => $response->get_error_message() ] );
            throw new \Exception( esc_html( __( 'No se pudo conectar con OpenAI API.', 'baloa-structure-auditor-seo' ) ) );
        }
        ...
        if ( isset( $data['error'] ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'OpenAI API returned an error', [ 'error' => $data['error'] ] );
            throw new \Exception( esc_html( $data['error']['message'] ?? __( 'Error devuelto por la API de OpenAI.', 'baloa-structure-auditor-seo' ) ) );
        }
        ...
        if ( ! is_array( $result ) || ! isset( $result['recommendations'] ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'OpenAI API returned invalid JSON schema', [ 'raw_content' => $content ] );
