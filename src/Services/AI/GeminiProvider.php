<?php
/**
 * BaloaStructureAuditorSEO\Services\AI\GeminiProvider
 *
 * Google Gemini 1.5 Pro provider for AI recommendations.
 */
...
class GeminiProvider implements AIProviderInterface {
    ...
    public function get_recommendations( string $url, array $context = [] ): array {
        ...
        if ( is_wp_error( $response ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'Gemini API request failed', [ 'error' => $response->get_error_message() ] );
            throw new \Exception( esc_html( __( 'No se pudo conectar con la API de Google Gemini.', 'baloa-structure-auditor-seo' ) ) );
        }
        ...
        if ( isset( $data['error'] ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'Gemini API returned an error', [ 'error' => $data['error'] ] );
            throw new \Exception( esc_html( $data['error']['message'] ?? __( 'Error devuelto por la API de Google Gemini.', 'baloa-structure-auditor-seo' ) ) );
        }
        ...
        if ( ! is_array( $result ) || ! isset( $result['recommendations'] ) ) {
            \BaloaStructureAuditorSEO\Core\Logger::error( 'Gemini API returned invalid JSON schema', [ 'raw_content' => $content ] );
    ...
    /**
     * Build standard guidelines and schema validation for the instructions.
     */
...
Responde ÚNICAMENTE con el objeto JSON válido. No agregues markdown adicional fuera del JSON.";
    }
